...
{
    /// <summary>
    /// Factory for creating comprehensive test data sets for various testing scenarios.
    /// Provides methods to generate realistic test data for performance, load, and compliance testing.
    /// </summary>
    public static class TestDataFactory
    {
        ...
        public static (Bus bus, List<Maintenance> MaintenanceHistory) CreateVehicleWithHistory(int days, string suffix = "")
        {
            ...
            // Create maintenance records over the specified period
            for (int i = 0; i < days / 30; i++) // One maintenance per month
            {
                var maintenanceDate = DateTime.Today.AddDays(-i * 30);
        ...
        public static Route CreateComplexRoute(int stops, int? busId = null, int? DriverId = null, string suffix = "")
        {
            ...
            var route = new Route
            {
                ...
                // AM Route data
                AMBeginMiles = GetRandomMileage(50000, 100000),
                AMEndMiles = 0, // Will be calculated
                ...
                Notes = $"Complex route with {stops} stops serving diverse neighborhoods"
            };

            // Calculate realistic mileage based on stops
            var routeMiles = stops * 2; // 2 miles per stop average
            route.AMEndMiles = route.AMBeginMiles + routeMiles;
            route.PMBeginMiles = route.AMEndMiles;
            route.PMEndMiles = route.PMBeginMiles + routeMiles;
        ...
        public static List<Route> CreateRouteHistory(List<int> busIds, List<int> driverIds, DateTime startDate, DateTime endDate)
        {
            ...
            {
                // Skip weekends for school routes
                if (currentDate.DayOfWeek != DayOfWeek.Saturday && currentDate.DayOfWeek != DayOfWeek.Sunday)
                {
                    ...
                    for (int i = 0; i < Math.Min(busIds.Count, driverIds.Count); i++)
                    {
                        var route = new Route
                        {
                            ...
                            AMEndMiles = 50025 + (i * 1000),
                            AMRiders = GetRandomRiderCount(5),

                            PMBeginMiles = 50025 + (i * 1000),
                            PMEndMiles = 50050 + (i * 1000),
                            PMRiders = GetRandomRiderCount(5)
                        };
                        routes.Add(route);
                    }
                }
        ...
        #region Driver Test Data

        /// <summary>
        /// Creates a diverse set of drivers with varying certifications and experience levels.
        /// </summary>
        /// <param name="count">Number of drivers to create</param>
        /// <param name="prefix">Prefix for driver identification</param>
        /// <returns>List of drivers with realistic profiles</returns>
        public static List<Driver> CreateDriverCohort(int count, string prefix = "DRIVER")
        {
        ...
        public static List<Fuel> CreateFuelHistory(int busId, int months)
        {
            ...
            {
                // 2-3 fuel-ups per month
                var fuelsPerMonth = 2 + (month % 2);

                for (int fuel = 0; fuel < fuelsPerMonth; fuel++)
                {
                    var fuelDate = DateTime.Today.AddMonths(-month).AddDays(-(fuel * 10));
                    var fuelAmount = 40 + (fuel * 5) + (month % 10); // 40-60 gallons
                    var fuelCost = fuelAmount * (3.20m + (month * 0.05m)); // Price variation over time
                    ...
                    var fuelRecord = new Fuel
                    {
                        ...
                        Notes = fuel % 5 == 0 ? "Regular maintenance fuel-up" : null
                    };
                    fuelRecords.Add(fuelRecord);
                }
            }

            return fuelRecords.OrderBy(f => f.FuelDate).ToList();
        }

        #endregion

        #region Compliance Test Data

        /// <summary>
        /// Creates test data that meets FERPA and regulatory compliance requirements.
        /// </summary>
        /// <param name="recordCount">Number of compliant records to create</param>
        /// <returns>Collection of compliant test data</returns>
        public static ComplianceTestDataSet CreateComplianceTestData(int recordCount)
        {
            var vehicles = CreateVehicleFleet(recordCount, "COMP");
            var drivers = CreateDriverCohort(recordCount, "COMP");

            // Ensure all drivers have proper certifications
        ...
        private static string GetRandomVehicleMake()
        {
            var makes = new[] { "Ford", "Chevrolet", "International", "Blue Bird", "Thomas Built", "IC Bus" };
            return makes[new Random().Next(makes.Length)];
        }

        private static string GetRandomVehicleModel()
        {
            var models = new[] { "Transit", "Express", "3000", "Vision", "C2", "CE300" };
            return models[new Random().Next(models.Length)];
        }

        private static int GetRandomVehicleYear()
        {
            return 2015 + new Random().Next(8); // 2015-2022
        }

        private static int GetRandomSeatingCapacity()
        {
            var capacities = new[] { 48, 54, 60, 66, 72, 78, 84, 90 };
            return capacities[new Random().Next(capacities.Length)];
        }

        private static string GetRandomMaintenanceType()
        {
            var types = new[] { "Oil Change", "Brake Service", "Tire Rotation", "Engine Inspection", "DOT Inspection", "Air Filter" };
            return types[new Random().Next(types.Length)];
        }

        private static decimal GetRandomMaintenanceCost()
        {
            return Math.Round(50m + (decimal)(new Random().NextDouble() * 500), 2); // $50-$550
        }
        ...
        private static int GetRandomRiderCount(int maxStops)
        {
            return new Random().Next(10, Math.Min(90, maxStops * 15)); // Realistic rider count
        }
        ...
        private static string GenerateVIN()
        {
            ...
            return new string(Enumerable.Repeat(chars, 17).Select(s => s[random.Next(s.Length)]).ToArray());
        }

        private static string GenerateLicenseNumber()
        {
            var random = new Random();
            return $"{(char)('A' + random.Next(26))}{(char)('A' + random.Next(26))}{random.Next(1000):D3}";
        }
    ...
    /// <summary>
    /// Data structure for compliance test data sets.
    /// </summary>
