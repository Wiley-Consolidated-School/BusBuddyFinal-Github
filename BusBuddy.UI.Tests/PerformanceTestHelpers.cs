...
{
    /// <summary>
    /// Helper utilities for performance testing and measurement.
    /// Provides methods to measure execution time, memory usage, and validate performance criteria.
    /// </summary>
    public static class PerformanceTestHelpers
    {
        ...
        /// <returns>TimeSpan representing execution duration</returns>
        ...
        /// <summary>
        /// Validates that an action completes within the specified time limit.
        /// </summary>
        /// <param name="action">The action to test</param>
        /// <param name="maxDuration">Maximum allowed duration</param>
        /// <param name="description">Description for assertion messages</param>
        public static void AssertPerformance(Action action, TimeSpan maxDuration, string description = "Operation")
        {
            var duration = MeasureExecutionTime(action);
            Assert.True(duration <= maxDuration,
                $"{description} took {duration.TotalMilliseconds}ms, expected <= {maxDuration.TotalMilliseconds}ms");
        }

        /// <summary>
        /// Validates that an async action completes within the specified time limit.
        /// </summary>
        /// <param name="action">The async action to test</param>
        /// <param name="maxDuration">Maximum allowed duration</param>
        /// <param name="description">Description for assertion messages</param>
        public static async Task AssertPerformanceAsync(Func<Task> action, TimeSpan maxDuration, string description = "Operation")
        {
        ...
        /// <param name="description">Description for assertion messages</param>
        public static void AssertMemoryUsage(Action action, int maxMemoryIncreaseMB, string description = "Operation")
        {
            var beforeMemory = GetMemoryUsage();
            action();
            var afterMemory = GetMemoryUsage();

            var memoryIncreaseMB = (afterMemory - beforeMemory) / (1024 * 1024);
            Assert.True(memoryIncreaseMB <= maxMemoryIncreaseMB,
                $"{description} used {memoryIncreaseMB}MB, expected <= {maxMemoryIncreaseMB}MB");
        }

        /// <summary>
        /// Converts bytes to megabytes for readable output.
        /// </summary>
        /// <param name="bytes">Memory in bytes</param>
        /// <returns>Memory in megabytes</returns>
        ...
        /// <summary>
        /// Creates a simple performance report for test output.
        /// </summary>
        /// <param name="operationName">Name of the operation tested</param>
        ...
        public static string CreatePerformanceReport(string operationName, TimeSpan duration, long memoryUsed)
        {
            return $"Performance Report - {operationName}:\n" +
